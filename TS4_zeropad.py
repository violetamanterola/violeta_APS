# -*- coding: utf-8 -*-
"""
Created on Thu Apr 24 20:10:58 2025
...
nn=N
#%%Datos simulación
w0=fs/4 #mitad de banda digital, osea que puede ser tambien fs/4
SNR=10 #dB (potencia de rudio cauntizado)
...
signal_1_fft = (1/N)*np.fft.fft(signal_1,n=N2,axis=0)
signal_1_abs = np.abs(signal_1_fft)

ventana2 = signal.windows.flattop(N).reshape(N,1)
signal_2 = sr * ventana2
signal_2_fft = (1/N)*np.fft.fft(signal_2,n=N2,axis=0)
signal_2_abs = np.abs(signal_2_fft)

ventana3 = signal.windows.hamming(N).reshape(N,1)
signal_3 = sr * ventana3
signal_3_fft = (1/N)*np.fft.fft(signal_3,n=N2,axis=0)
signal_3_abs = np.abs(signal_2_fft)
#%% señal rectangular sin ventana
signal_0 = sr
signal_0_fft = (1/N)*np.fft.fft(signal_0, n=N2, axis=0)
...
#%%estimador de freecunecia 
k_0=np.argmax(signal_0_abs[:N2//2, :],axis =0 )
k_1=np.argmax(signal_1_abs[:N2//2, :],axis =0 )
k_2=np.argmax(signal_2_abs[:N2//2, :],axis =0 )
k_3=np.argmax(signal_3_abs[:N2//2, :],axis =0 )
...
#ahora vpy a ver una variaci+on uiforme entre dnjvwi
# plt.figure(figsize=(10, 6))
plt.figure(1)
plt.hist(o0, bins=30, alpha=0.5, color = 'green',label='Rectangular-sin ventana')
plt.hist(o1, bins=30, alpha=0.5, label='Blackman')
plt.hist(o2, bins=30, alpha=0.5, label='flattop')
# plt.hist(o3, bins=30, alpha=0.5, label='hamming')
